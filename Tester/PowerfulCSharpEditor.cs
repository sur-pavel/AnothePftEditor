...
namespace PftEditor
{
    public partial class PowerfulCSharpEditor : Form
    {
        ...
        public static Dictionary<string, string> _unifors = new Dictionary<string, string>()
        {
            ...
            {
                "uf('+99^')",
                "Групповая установка глобальных переменных. Применяется для ИРБИС-Навигатора.\n" +
                "Исходная строка закодирована с помощью URL-кодировки.\n" +
                "После раскодировки рассматривается как список строк.\n" +
                "Каждая строка этого списка имеет структуру: NNN#<значение_глобальной_переменной_NNN><tt>."
            },

            {"uf('+0^')", "Выдать содержимое документа полностью (формат ALLl)."},

            {"uf('++0^')", "Выдать содержимое документа полностью (формат ALLl)."},

            {"uf('+@^')", "Выдать содержимое документа полностью (формат ALLl)."},

            {"uf('+N^')", "Выдать содержимое документа полностью в формате JSON."},

            {"uf('A^')", "Выдать количество повторений поля, метка которого указана после идентификатора функции."},

            {"uf('P^')", "Выдать заданное повторение поля."},

            {"uf('+E^')", "Выдать заданное оригинальное повторение поля."},
            ...
            {
                "uf('I^')",
                "Возвращает значение из справочника,\n" +
                "соответствующее переданному коду (иными словами, осуществляется раскодировка)."
            },

            {"uf('1^')", "Вернуть параметр из INI-файла."},

            {"uf('J^')", "Вернуть заданный подэлемент."},

            {"uf('D^')", "Вернуть кол-во ссылок для заданного термина."},

            {"uf('7^')", "Форматирование документа из другой БД\n" +
...

            {
                ...
                "Функция обеспечивает возможность связать запись с рядом других записей по какому бы то ни было общему признаку.\n" +
                "К примеру, можно отобрать все записи с определенным заглавие, индексом УДК/ББК, ключевым словом."
            },

            {
                "uf('+F^')",
                "Команда постредактуры: очистить результат расформатирования от двойных разделителей\n" +
                "(двойных точек или двойных конструкций <. – >).\n" +
            ...
            {"uf('+1I^')", "Групповая мультираскодировка списка."},

            {"uf('+1V^')", "Исключение неоригинальных значений из списка."},

            {"uf('+I^')", "Сортировка списка."},

            {"uf('+6^')", "Формирования ссылки (гиперссылки)."},
            ...
            {"uf('2^')", "Возвращает имя текущей базы данных."},

            {
                "uf('+9V^')",
                "Возвращает последний номер MFN в текущей базе данных, увеличенный на единицу (MAX_MFN) + 1).\n" +
                "В общем случае параметр имеет вид 2N,\n" +
                "где N - выводимое количество символов, обрезанное до необходимой длинны справа."
            },

            {
                "uf('4N^')",
                "Возвращает поколение системы, в которой осуществляется расформатирование.\n" +
                "Может быть полезен при разработке единых форматов,\n" +
                "которые по-разному выполняются в ИРБИС 32 и ИРБИС 64."
            },

            {"uf('+4^')", "Форматировать предыдущую копию текущей записи."},
            ...
            {"uf('B^')", "Вернуть номер текущего повторения в повторяющейся группе."},

            {"uf('C^')", "Выдать библиографическую свертку документа."},

            {
                "uf('L^')",
                "Контроль ISSN/ISBN. Возвращаемое значение: 0 – при положительном результате, 1 – при отрицательном."
            },

            {"uf('U^')", "Вернуть окончание термина."},

            {"uf('V^')", "Кумуляция номеров журналов."},

            {"uf('W^')", "Декумуляция номеров журналов."},

            {"uf('O^')", "Контроль кумуляции."},

            {"uf('Y^')", "Вывод сведений о всех экземплярах по всем местам хранения."},

            {"uf('+91^')", "Возвращает данные о всех свободных (не выданных) экземплярах по всем местах хранения."},

            {"uf('+92^')", "Вернуть имя файла из заданного полного пути/имени."},

            {"uf('+93^')", "Вернуть путь из заданного полного пути/имени."},

            {"uf('+94^')", "Вернуть расширение из заданного полного пути/имени."},

            {"uf('+9L^')", "Вернуть имя диска из заданного полного пути/имени."},

            {"uf('+9C^')", "Проверить наличие файла/корректность URL."},

            {"uf('+9A^')", "Вставить данные из заданного текстового файла."},

            {"uf('+9D^')", "Вернуть размер файла в байтах."},

            {"uf('+9J^')", "Сохранить заданный внутренний двоичный объект в заданном файле."},

            {
                "uf('+9K^')",
                "Представить заданный двоичный файл в виде: ^A<тип_файла>^B<данные файла перекодированные на основе URLEncode>."
            },

            {
                "uf('Z^')",
                "Удалить заданный файл. Если имя файла задано в виде маски, то удаляться будут все соответствующие маске файлы."
            },
            ...
            {
                "uf('6^')",
                "Отсортировать повторения заданного поля (имеется в виду строковая сортировка) – функция ничего не возвращает.\n" +
                "Можно применять только в глобальной корректировке."
            },

            {"uf('+2^')", "Выполнить формат."},

            {"uf('+8^')", "Выполняет внешнее приложение. Всё, что после +2 – параметры командной строки."},

            {"uf('S^')", "Вызвать функцию из внешней DLL."},

            {"uf('R^')", "Универсальный счетчик."},

            {"uf('[^')", "Генерация случайного числа."},

            {"uf('+3G^')", "Чистка данных от команд контекстного выделения."},
            ...
            {
                "uf('+1^')",
                "\nФункция предназначена для обрезания перед помещением в словарь базы данных значения,\n" +
                "формируемого путём соединения префикса TXT= и содержимого подполя ^B ссылки на\n" +
                "текстовый файл полнотекстовой базы."
            },

            {"uf('+1R^')", "Очистить (опустошить) все глобальные переменные."},

            {"uf('+1W^')", "Чтение глобальных переменных."},

            {"uf('+1K^')", "Запись в глобальные переменные."},

            {"uf('+1M^')", "Групповая мультираскодировка переменных."},

            {"uf('+1S^')", "Перемножение двух списков (групп переменных)."},

            {"uf('+1A^')", "Вычитание списков (групп переменных)."},

            {"uf('+1G^')", "Сложение списков (групп переменных)."},

            {"uf('+1T^')", "Исключение неоригинальных значений из группы переменных."}
        };
